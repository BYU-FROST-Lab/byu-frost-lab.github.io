...
After adding or updating dependencies, run `docker compose up --build --renew-anon-volumes` so the container's `node_modules` volume is recreated with the new packages.
